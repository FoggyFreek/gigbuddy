...
export async function fetchCredentialRecord(executor, tenantId, type) {
  const { encrypted, retained } = columnsFor(type)
  const { rows } = await executor.query(
    `SELECT ${encrypted} AS encrypted_value,
...
export async function fetchCredentialStatus(executor, tenantId, type) {
  const { encrypted, changedAt, retained } = columnsFor(type)
  ...
  const { rows } = await executor.query(
    `SELECT (${encrypted} IS NOT NULL)${retainedGuard} AS is_set,
...
export async function storeEncryptedCredential(executor, tenantId, type, envelope) {
  ...
  const { rows } = await executor.query(
    `INSERT INTO tenant_integrations
       (tenant_id, ${encrypted}, ${changedAt}${retainedColumn})
     VALUES ($1, $2::jsonb, NOW()${retainedValue})
