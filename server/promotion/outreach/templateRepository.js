export async function listTemplates(executor, tenantId, limit) {
  const { rows } = await executor.query(
    `SELECT id, name, subject, locale, origin_key, created_at, updated_at
...
export async function insertTemplate(executor, tenantId, data) {
  const { rows } = await executor.query(
    `INSERT INTO outreach_templates
      (tenant_id, name, subject, preview_text, body_json, body_html, body_text, origin_key, locale)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
    [tenantId, data.name, data.subject, data.previewText, data.bodyJson, data.bodyHtml,
      data.bodyText, data.originKey, data.locale],
  )
  return rows[0]
}
...
export async function copyTemplate(executor, templateId, tenantId, name) {
  const { rows } = await executor.query(
    `INSERT INTO outreach_templates
      (tenant_id, name, subject, preview_text, body_json, body_html, body_text, origin_key, locale)
     SELECT tenant_id, $3, subject, preview_text, body_json, body_html, body_text, origin_key, locale
       FROM outreach_templates
