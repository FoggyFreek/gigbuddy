import { computePurchaseLineTotals } from '../purchases/purchaseTotals.ts'
...
type PostFn = (code: string, side: 'debit' | 'credit', cents: number) => void

// One draft line's postings — see computeJournalEffects for the model.
function postFormLine(line: JournalForm['lines'][number], settings: AccountingSettings | null, post: PostFn): void {
  const code = line.account_code?.trim()
  const amount = Math.round(Number(line.amount_cents) || 0)
  if (!code || !line.side || amount <= 0) return
...
// Splits the netted totals into the two columns — see computeJournalEffects.
function effectColumns(totals: Map<string, number>, accounts: Account[]) {
...
export function computeJournalEffects(
  ...
  const post: PostFn = (code, side, cents) => {
    totals.set(code, (totals.get(code) ?? 0) + (side === 'debit' ? cents : -cents))
  }

  for (const form of forms) {
    for (const line of form.lines) {
      postFormLine(line, settings, post)
    }
  }

  const { debit, credit } = effectColumns(totals, accounts)
