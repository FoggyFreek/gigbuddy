...
export function outstandingCents(ret) {
  return Math.abs(ret.net_cents) - (ret.paid_cents ?? 0)
}
...
export function statusMeta(ret, today = new Date().toISOString().slice(0, 10)) {
  switch (ret.status) {
