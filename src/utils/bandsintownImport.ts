...
export function csvTicketTypeToAdmission(ticketType: string | null | undefined, ticketLink: string | null | undefined): 'free' | 'paid' {
...
function countNonEmpty(row: Record<string, unknown>): number {
...
interface RawRow extends ParsedBandsintownRow {
  _raw: Record<string, unknown>
}

export interface ParseBandsintownResult {
  rows: ParsedBandsintownRow[]
  parseError: string | null
}

export function parseBandsintownCsv(text: string): ParseBandsintownResult {
  ...
  const bySecondaryKey = new Map<string, RawRow>()
...
export function venueMatchScore(a: string | null | undefined, b: string | null | undefined): number {
  if (!a || !b) return 0
...
interface GigWithVenueNames extends Gig {
  event_date?: string
}

export function isLikelyDuplicate(row: ParsedBandsintownRow, existingGigs: GigWithVenueNames[]): boolean {
