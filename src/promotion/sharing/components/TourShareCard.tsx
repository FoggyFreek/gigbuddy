...
import { SHARE_FORMATS } from '../shareCard.ts'
import { venueCity, venueHeadline } from '../../../people/venues/venueDisplay.ts'
import PhotoBackdrop from './primitives/PhotoBackdrop.tsx'
import SocialsRow from './SocialsRow.tsx'

// Socials is a record of network keys to handle strings.
...
function calculateRowFontSize(count: number, availableHeight: number, rowMultiplier = 2.5): number {
  const MAX_FONT = 52
  const MIN_FONT = 10
...
function TourFrame({ format, photoSrc, photoOpacity, zoom, pan, children }: Readonly<TourFrameProps>) {
  ...
  return (
    ...
    >
      <div style={{ position: 'absolute', inset: 0, opacity: (photoOpacity ?? 55) / 100 }}>
        <PhotoBackdrop
          src={photoSrc}
          zoom={zoom}
          pan={pan}
...
const TourShareCard = forwardRef<HTMLDivElement, TourShareCardProps>(function TourShareCard(
  ...
  const year = yearProp ?? (gigs.length > 0
    ? new Date(String(gigs[0].event_date)).getFullYear()
    : new Date().getFullYear())

  const props: TourLayoutProps = { gigs, photoSrc, photoOpacity, zoom, pan, accent, year, today, socials, logoSrc, showBanners }
