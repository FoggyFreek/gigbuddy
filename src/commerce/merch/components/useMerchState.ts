...
import { listProducts } from '../merch.ts'
import { listAccounts, getAccountingSettings } from '../../../finance/accounts/accounts.ts'
...
// Loads the product catalogue; `reload` refreshes it after any mutation (stock
// changes on every merch action). Per-product sales totals live in the page's
// period-scoped summary state, not here. Accounts and settings only feed the
// product dialog's revenue-account picker, so they load once on mount and are
// not part of `reload`.
export function useMerchState(): UseMerchStateResult {
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([listProducts(), listAccounts(), getAccountingSettings()])
      .then(([p, accs, settings]) => {
        if (cancelled) return
        setProducts(p)
  ...
  return { products, revenueAccounts, error, setError, reload }
}
