...
describe('TaxesSection', () => {
  it('lists the flag and all rates as a table with column captions', () => {
    wrap()
    ...
    expect(within(table).getByText('Copyright / PRS %')).toBeInTheDocument()
    expect(screen.getByLabelText('Subject to VAT')).toBeChecked()
    expect(screen.getByLabelText('Ticket VAT %')).toHaveValue(9)
    expect(screen.getByLabelText('Copyright / PRS %')).toHaveValue(null)
  })
  ...
  it('greys out both rates instead of hiding them on a deal that carries no VAT', () => {
    ...
    expect(screen.getByLabelText('Copyright / PRS %')).toBeDisabled()
  })
  ...
  it('reports the flag and each rate to the form', async () => {
    ...
    await user.type(screen.getByLabelText('Copyright / PRS %'), '7')
    expect(onChange).toHaveBeenCalledWith('copyright_percentage', '7')
  })
})
