...
import { createEmailTemplate, deleteEmailTemplate, getEmailTemplate, updateEmailTemplate } from '../api/emailTemplates.ts'
import useDebouncedSave from '../hooks/useDebouncedSave.ts'
import { hasRequiredErrors } from '../utils/requiredFields.ts'
...
function downloadEml(name: string, subject: string, bodyHtml: string) {
  const content = [
    'MIME-Version: 1.0',
...
export default function EmailTemplateFormModal({ mode, templateId, onClose }: EmailTemplateFormModalProps) {
  ...
  const saveFn = useCallback(
    async (patch: Record<string, unknown>) => { await updateEmailTemplate(templateId!, patch) },
    [templateId]
  )
