...
export default function ChartOfAccountsSection() {
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      ...
      if (err.status === 409) setErrorId(deleteTarget.id)
      setDeleteTarget(null)
    } finally {
      ...
      {loading ? (
        ...
        groupedTrees.map(({ type, trees }) => (
          <Box key={type} sx={{ mb: 2 }}>
            <Typography variant="caption" fontWeight={700} color="text.secondary" sx={{ display: 'block', mb: 0.5, textTransform: 'uppercase', letterSpacing: 0.5 }}>
              {TYPE_LABELS[type]}
            </Typography>
            {trees.map((node) => (
              <AccountRow
