...
export default function MemberReimbursementCard({ member, expanded, purchases, onToggle, onRegister, onMarkReimbursed }) {
  return (
    <Paper variant="outlined" sx={{ p: 1.5 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0 }}>
          <Typography variant="body1" fontWeight={600} noWrap>{member.band_member_name}</Typography>
          <Chip size="small" label={`${member.outstanding_count} purchase${member.outstanding_count === 1 ? '' : 's'}`} sx={{ mt: 0.25 }} />
        </Box>
        <Typography variant="h6" fontWeight={700}>{formatEur(member.outstanding_cents)}</Typography>
      </Box>
...
MemberReimbursementCard.propTypes = {
