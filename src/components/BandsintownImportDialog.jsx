...
import { importGigs, listGigs } from '../api/gigs.js'
...
import {
  ...
  venueMatchScore,
  isLikelyDuplicate,
} from '../utils/bandsintownImport.js'

function buildInitialRowStates(rows, existingGigs) {
  return rows.map((row) => ({
    included: !isLikelyDuplicate(row, existingGigs),
    venueType: 'venue',
    selectedVenue: null,
    status: 'confirmed',
    isDuplicate: isLikelyDuplicate(row, existingGigs),
  }))
}

async function preFillVenueMatches(rows, setRowStates) {
  const seen = new Set()
  const uniqueNames = []
    ...
    rows.forEach((row, i) => {
      ...
      if (scored[0].score >= 0.4) {
        next[i] = {
          ...
          venueType: scored[0].venue.category,
        }
      }
    })
    return next
...
function buildGigPayload(row, state) {
  return {
...
export default function BandsintownImportDialog({ onClose }) {
  ...
  function updateRowState(index, patch) {
    setRowStates((prev) => {
      const next = [...prev]
  ...
  async function handleImport() {
    const payload = rows.reduce((acc, row, i) => {
    ...
    <Dialog open fullWidth maxWidth="xl">
                    ...
                    >
                      ...
                      <TableCell sx={{ minWidth: 280 }}>
                        <VenuePicker
                          ...
                          onChange={(v) => updateRowState(i, { selectedVenue: v })}
                        />
                      </TableCell>
                      <TableCell>
                        <TextField
      ...
      <DialogActions>
        ...
        {step === 'done' && (
          <Button
            variant="contained"
            onClick={() => onClose(result?.created > 0)}
          >
            Close
          </Button>
        )}
...
BandsintownImportDialog.propTypes = {
