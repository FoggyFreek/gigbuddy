...
import BannerMosaicCard from './share/BannerMosaicCard.jsx'
import {
...
export default function BannerMosaicDialog({ open, onClose, gigs = [] }) {
  ...
  const availableYears = useMemo(() => {
    const years = new Set(
      gigsWithBanners.map((g) => new Date(g.event_date).getFullYear()),
    )
  ...
  const filteredGigs = useMemo(() => {
    ...
    return gigsWithBanners.filter(
      (g) => new Date(g.event_date).getFullYear() === selectedYear,
    )
  ...
  async function handleDownload() {
    ...
    try {
      ...
      downloadBlob(blob, buildBannerMosaicFilename(selectedYear, format))
    } catch (e) {
