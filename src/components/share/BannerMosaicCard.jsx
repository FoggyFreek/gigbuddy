...
function buildRowsForCount(gigs, aspectRatios, rowCount, frameWidth, frameHeight) {
  const rows = []
...
function scoreRows(rows, frameWidth, frameHeight) {
  ...
  return { baseHeights, baseHeight, scale, usedArea }
}

function buildMosaicLayout(gigs, aspectRatios, format) {
...
function BannerImage({ gig, style, onImageLoad }) {
  return (
    <img
...
BannerImage.propTypes = {
  gig: PropTypes.shape({
    banner_path: PropTypes.string.isRequired,
  }).isRequired,
  style: PropTypes.object,
  onImageLoad: PropTypes.func,
}

const BannerMosaicCard = forwardRef(function BannerMosaicCard({ gigs, format, backgroundColor = '#000' }, ref) {
  const { width, height } = SHARE_FORMATS[format]
  const [aspectByPath, setAspectByPath] = useState({})
  ...
  const handleImageLoad = useCallback((gig, e) => {
...
BannerMosaicCard.propTypes = {
  ...
  format: PropTypes.oneOf(['square', 'story']).isRequired,
