import type { StickerId } from './stickerConfigs.ts'
import { STICKER_CONFIGS } from './stickerConfigs.ts'

type StickerPosition = 'left-top' | 'right-top' | 'left-bottom' | 'right-bottom'

const POSITION_STYLES: Record<StickerPosition, Record<string, number>> = {
...
const POSITION_ROTATION: Record<StickerPosition, number> = {
  'left-top':     -15,
  'right-top':    15,
  'left-bottom':  15,
  'right-bottom': -15,
...
export default function StickerOverlay({ sticker, position = 'right-top', accent = '#f5c542' }: StickerOverlayProps) {
  ...
  const pos = POSITION_STYLES[position as StickerPosition] ?? POSITION_STYLES['right-top']
  const rotation = POSITION_ROTATION[position as StickerPosition] ?? 10
