...
import { tooltipLeft } from '../../utils/chartTooltipPosition.js'
...
export default function ResultChartTooltip() {
  ...
  const cardRef = useCallback((node) => {
    ...
    observerRef.current = new ResizeObserver((entries) => {
      setCardWidth(Math.ceil(entries[0].target.offsetWidth))
    })
  ...
  const lineX = (xScale(axis.axisValue) ?? 0) + (xScale.bandwidth?.() ?? 0) / 2
