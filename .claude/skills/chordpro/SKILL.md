...
| Diagram placement | `{diagrams}`, `{diagrams: on\|top\|bottom\|right\|left\|below\|off}` | Full, plus a GigBuddy-only `left`. `on` and a bare directive mean `bottom`; `below` inserts the grid at that source position; `off` suppresses screen and print grids. `right`/`left` put the grid in a side column beside the lyrics, stacked vertically. |
...
**`{diagrams}`** controls the auto-generated grid of used chords: `on`/`top`/`bottom`/`right`/`left`/`below`/`off`. Bare/on resolves to `bottom`; `below` emits an inline placement block; `right` and `left` wrap the flow in a two-column `cp-side-layout` grid whose diagram column stacks vertically (`cp-diagrams-vertical`). Below `sm` (or in a SplitView pane — `ChordProView`'s `compact` prop) that grid collapses to one column and the diagrams wrap horizontally again on screen; print keeps the side column either way. `ChordProView` shows a collapsible diagram grid on screen and a print version. Undefined auto-grid chords are omitted, while an inline `{chord}` can still render its name without a shape.
