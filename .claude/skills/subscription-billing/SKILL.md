...
- **Blockers** (`computeModuleBlockers`, `moduleCapacityService.js`) check member/storage limits against **ALL owned tenants of that kind, archived included**; only the **band cap** counts active tenants. Re-run under the exact lock set every capacity-growing write takes → 409 `over_target_limit`.
