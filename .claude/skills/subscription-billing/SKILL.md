...
- **Provider port**: `server/commerce/billing/paymentProvider/` — code imports `getPaymentProvider()`, never a concrete adapter, and speaks **canonical statuses only** (`statuses.js`, including `REFUND_STATUS`). Tests inject `src/tests/server/_fakeProvider.js`, which throws a **real `PaymentProviderError`** so `retryable: false` is actually exercised.
- **Trial** — 30 days, **once per USER** (`hasUsedTrial(db, userId)`), **Gold only**, one starter module. **No mandate, no provider object, no payment at all.** A second module added during the trial is free and does **not** extend it. A trial that lapses is cancelled by `reconcileExpiredTrials`; the canceled row is what keeps the trial spent.
- **Conversion** — `checkout` prices the module set for the chosen interval and creates **ONE payment** (`sequenceType: first`) that both takes the first full combined amount and establishes the mandate. There is no €0.01 verification charge, and no `pending_mandate` status.
