...
# Backend layering: vertical slices

Every backend feature owns its route, service, repository, and validators inside one domain slice. The **canonical example** to copy is the rehearsals stack:

- `server/planning/rehearsals/rehearsals.js` — thin HTTP layer
- `server/planning/rehearsals/rehearsalService.js` — domain logic
- `server/planning/rehearsals/rehearsalRepository.js` — SQL
- `server/planning/rehearsals/rehearsalValidators.js` — pure input parsing

Read those files before writing or refactoring backend code; match their style exactly.

## Layer responsibilities

### Route (`server/<domain>/<feature>/<feature>.js`)
...
- Register new routers in `server/app/apiRouter.js`.

### Service (`server/<domain>/<feature>/<resource>Service.js`)
...
### Repository (`server/<domain>/<feature>/<resource>Repository.js`)
...
### Validators (`server/<domain>/<feature>/<resource>Validators.js`)
